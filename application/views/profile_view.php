...
<div class="flex-1 max-w-2xl w-full mx-auto px-4 py-6">
    ...
    <div class="h-36 sm:h-48 w-full relative bg-gradient-to-r from-red-950/40 to-slate-900 overflow-hidden">
        <?php if (!empty($user['banner'])): ?>
            <img src="<?= assets_url($user['banner']); ?>" alt="User Banner" class="w-full h-full object-cover">
        <?php else: ?>
        ...
        <div class="flex flex-col sm:flex-row sm:items-end justify-between gap-4 mb-4">
            ...
            <div class="relative w-24 h-24 sm:w-28 sm:h-28 flex-shrink-0 mx-auto sm:mx-0">
                <div class="w-full h-full rounded-full p-[2.5px] bg-slate-950 ring-2 ring-white/[0.08] overflow-hidden">
                    <img src="<?= !empty($user['avatar']) ? assets_url($user['avatar']) : assets_url('default.jpg'); ?>" 
            ...
            <div class="text-center sm:text-right flex flex-col items-center sm:items-end gap-2">                    
                <button onclick="alert('Fitur Segera Hadir')" class="bg-white/[0.05] hover:bg-red-600 text-slate-200 hover:text-white text-[11px] font-semibold px-4 py-2 rounded-xl border border-white/[0.06] transition-all duration-300">
                    Edit Profil
                </button>
            </div>
...
<script>
    ...
    const userId = <?= isset($user['id_user']) ? (int)$user['id_user'] : 0; ?>;
        ...
        fetch(url)
            ...
            .then(data => {
                ...
                data.forEach(post => {
                    ...
                    const cardHTML = `
                        <article class="glass-card rounded-xl overflow-hidden group transition-all duration-300 relative hover:bg-white/[0.02]">
